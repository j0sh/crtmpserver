...
bool ResolvePath(string path, vector<string> &result) {

    glob_t g;
    if (glob(STR(path), GLOB_TILDE, NULL, &g) != 0) {
        FATAL("glob failed: %s (%d)", strerror(errno), errno);
        globfree(&g);
        return false;
    }

    for (uint32_t i = 0; i < g.gl_pathc; i++) {
        //FINEST("%s", g.gl_pathv[i]);
        ADD_VECTOR_END(result, g.gl_pathv[i]);
    }
