...
bool InboundHTTP4RTMP::SignalInputData(IOBuffer &buffer) {
	...
	//2. Get the HTTP far protool and test to see if it has ContentLength
	//    InboundHTTPProtocol *pHTTP = (InboundHTTPProtocol *) _pFarProtocol;
	//    DeleteNearProtocol(false);
	//    if (pHTTP == NULL || pHTTP->GetContentLength() == 0) {
	//        FATAL("Invalid HTTP request");
	...
	//    }
	//
	//    //3. Test it and see if all the data was transfered
	//    if (!pHTTP->TransferCompleted()) {
	//        FINEST("Transfer not completed yet");
	//        return true;
	//    }
	//
	//    //4. Get the HTTP request
	//    Variant request = pHTTP->GetHeaders();
	//
	//    //5. Get the URL
	//    string url = request[HTTP_FIRST_LINE][HTTP_URL];
	//    FINEST("protocol: %d; url: %s", GetId(), STR(url));
	//
	//    //6. split it in meaningful parts
	...
	//
	//    if (parts[1] == "fcs") {
	//        result = ProcessFcs(buffer, parts);
	//    } else if (parts[1] == "open") {
	//        result = ProcessOpen(buffer, parts);
	//    } else if (parts[1] == "idle") {
	//        result = ProcessIdle(buffer, parts);
	//    } else if (parts[1] == "send") {
	//        result = ProcessIdle(buffer, parts);
	//    } else {
	//        FATAL("Invalid command: %s", STR(parts[1]));
	...
	//    //    if (parts[1] == "fcs") {
	//    //        //        FINEST("%s:%d --> %s:%d",
	//    //        //                STR(((TCPCarrier *) GetIOHandler())->GetFarEndpointAddress()),
	//    //        //                ((TCPCarrier *) GetIOHandler())->GetFarEndpointPort(),
	//    //        //                STR(((TCPCarrier *) GetIOHandler())->GetNearEndpointAddress()),
	//    //        //                ((TCPCarrier *) GetIOHandler())->GetNearEndpointPort());
	//    //        _outputBuffer.PutString(((TCPCarrier *) GetIOHandler())->GetNearEndpointAddress() + "\n");
	//    //        buffer.Ignore(pHTTP->GetContentLength());
	//    //        //pHTTP->SetDisconnectAfterTransfer(true);
	//    //        return BaseProtocol::EnqueueForOutbound();
	//    //    } else if (parts[1] == "open") {
	//    //        //8. Some minimal validation...
	//    //        if (parts.size() != 3) {
	//    //            FATAL("Invalid request:\n%s", STR(request.ToString()));
	//    //            return false;
	//    //        }
	...
	//    //        }
	//    //
	//    //        //9. create a SID, save it, and send it after that
	//    //        string sid = md5(format("%d", GetId()), true).substr(0, 16);
	//    //        _generatedSids[sid] = sid;
	//    //        _outputBuffer.PutString(sid + "\n");
	//    //
	//    //        //10. We are done
	//    //        buffer.Ignore(pHTTP->GetContentLength());
	//    //        return BaseProtocol::EnqueueForOutbound();
	//    //    } else if ((parts[1] == "idle") || (parts[1] == "send")) {
	//    //        //12. Some minimal validation
	//    //        if (parts.size() != 4) {
	//    //            FATAL("Invalid request:\n%s", STR(request.ToString()));
	//    //            return false;
	//    //        }
	...
	//    //        }
	//    //
	//    //        //13. Bind to the target Inbound RTMP protocol
	//    //        InboundRTMPProtocol *pRTMPProtocol = NULL;
	//    //        if (_pNearProtocol == NULL) {
	//    //            //14. This might be a new connection. Do we have that sid generated?
	//    //            if (!MAP_HAS1(_generatedSids, parts[2])) {
	//    //                FATAL("Invalid sid: %s", STR(parts[2]));
	//    //                return false;
	//    //            }
	//    //
	//    //            //15. See if we have to generate a new connection or we just pick up
	//    //            //a disconnected one
	//    //            if (MAP_HAS1(_protocolsBySid, parts[2])) {
	//    //                pRTMPProtocol = (InboundRTMPProtocol *) ProtocolManager::GetProtocol(_protocolsBySid[parts[2]]);
	//    //            } else {
	//    //                pRTMPProtocol = new InboundRTMPProtocol();
	//    //                _protocolsBySid[parts[2]] = pRTMPProtocol->GetId();
	//    //                SetNearProtocol(pRTMPProtocol);
	//    //                pRTMPProtocol->SetFarProtocol(this);
	//    //            }
	//    //        } else {
	//    //            pRTMPProtocol = (InboundRTMPProtocol *) _pNearProtocol;
	//    //        }
	//    //
	//    //        if (pRTMPProtocol == NULL) {
	//    //            FATAL("Unable to fine the corresponding RTMP protocol");
	//    //            return false;
	//    //        }
	//    //
	//    //        _outputBuffer.PutByte(1);
	//    //        buffer.Ignore(1);
	//    //
	//    //        if (parts[1] == "send") {
	//    //            if (!pRTMPProtocol->SignalInputData(buffer)) {
	//    //                FATAL("Unable to signal RTMP protocol");
	//    //                return false;
	//    //            }
	//    //        }
	//    //
	//    //        IOBuffer *pBuffer = pRTMPProtocol->GetOutputBuffer();
	//    //        if (pBuffer != NULL) {
	//    //            FINEST("We have to send %d bytes", GETAVAILABLEBYTESCOUNT(*pBuffer));
	//    //            _outputBuffer.PutBuffer(GETIBPOINTER(*pBuffer), GETAVAILABLEBYTESCOUNT(*pBuffer));
	//    //        }
	//    //
	//    //        return BaseProtocol::EnqueueForOutbound();
	//    //    } else {
