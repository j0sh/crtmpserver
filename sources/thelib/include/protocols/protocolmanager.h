...
public:
	/*!
		Function that registers the protocol. If the protocol is already active or dead, this function will do nothing.
		@param pProtocol: The pointer to the protocol to be registered.
	*/
	static void RegisterProtocol(BaseProtocol *pProtocol);
	/*!
		Function that ters erases protocol.
		@param pProtocol: The pointer to the protocol to be erased.
	*/
	static void UnRegisterProtocol(BaseProtocol *pProtocol);
	/*!
		Function which deletes the protocol. This is called during shutdown to delete the protocol and NULL the application set for the protocol.
		@param pProtocol: The pointer to the protocol to be erased.
	*/
	static void EnqueueForDelete(BaseProtocol *pProtocol);
	/*!
		This function makes sure that the memory allocated to all dead protocols are deleted.
	*/
	static uint32_t CleanupDeadProtocols();
	/*!
		This function deletes all active protocols.
	*/
	static void Shutdown();
	/*!
		Returns the BaseProtocol pointer to the protocol referred to by the provided id.
	...
	*/
	...
	/*!
		Returns the vector that contains all active protocols
	*/
