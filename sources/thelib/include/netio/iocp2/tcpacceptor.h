...
#define ACCEPT_OV_RAW_BUFFER 1024

class BaseClientApplication;

typedef struct _AcceptOV : public GenericOV {
    SOCKET acceptedSocket;
    uint8_t rawBuffer[ACCEPT_OV_RAW_BUFFER];
    uint32_t rawBufferLength;
    uint32_t rawBufferWritten;
} AcceptOV;
...
private:
	...
	Variant _parameters;
public:
    TCPAcceptor(string ipAddress, uint16_t port, Variant parameters, vector<uint64_t>/*&*/ protocolChain);
    virtual ~TCPAcceptor();
    bool StartAccept(BaseClientApplication *pApplication);
	virtual bool SignalOutputData();
    virtual bool OnEvent(GenericOV *pGenericOV);
    virtual bool OnConnectionAvailable(GenericOV *pGenericOV);
    AcceptOV * GetAcceptOV();
